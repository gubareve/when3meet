            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" action="#" bind:this={loginForm}>
                    <div>
                        ...
                        <Input type="email" bind:value={email} label="Email" id="login-email" placeholder="john.doe@example.com"/>
                        {#if error}
                    ...
                    <div>
                        ...
                        <Input type="password" bind:value={password} label="Password" id="login-password" placeholder = "Password"/>
                    </div>
                    <Button type="submit" on:click={login}>{#if loading}<Spinner class="mr-3" size="4" color="white" />{/if}Login</Button>
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        Need an account? <a href="/register" class="font-medium text-primary-600 hover:underline dark:text-primary-500">Register here</a>
                    </p>
                </form>
            </div>
        </div>
