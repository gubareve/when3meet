<script lang="ts">
    ...
    async function save() {
        ...
        for (let [i, day] of schedule.entries()) {
            ...
            if (s) {
                // Update
                promises.push(pb.collection("schedules").update(s.id, {
                    available: vals,
                }));
            } else {
                promises.push(pb.collection("schedules").create({
                    author: data.user.id,
                    day: data.days[i].id,
                    available: vals,
                }))
...
<div class="container max-w-2xl mx-auto pt-10">
    ...
    <TimeViewInteractive interactive startTime={data.meeting.start} endTime={data.meeting.end} dayNames={data.days.map(dateToString)} bind:schedule={schedule} initialSchedule={schedule} />
